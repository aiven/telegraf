...
- clickhouse_replication_queue (see [system.replication_queue][] for details)
  ...
  - fields:
    ...
    - num_merge_parts_ttl_delete (Number of TTLDelete MERGE_PARTS
      replication queue items)
    - num_merge_parts_ttl_recompress (Number of TTLRecompress MERGE_PARTS
      replication queue items)
    - num_mutate_part (Number of MUTATE_PART replication queue items)
    - too_many_tries_replicas (Number of replication queue items with
      num_tries > 100)
    - num_tries_replicas (Number of replication queue items with
      num_tries > 1)
